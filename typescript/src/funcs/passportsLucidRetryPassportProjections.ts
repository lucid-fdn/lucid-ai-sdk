 ...
 */
export function passportsLucidRetryPassportProjections(
  client: RaijinLabsLucidAiCore,
  request: operations.LucidRetryPassportProjectionsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.LucidRetryPassportProjectionsResponse,
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.LucidRetryPassportProjectionsRequest$outboundSchema,
        value,
      ),
  ...
  const path = pathToFunc("/v1/passports/{passport_id}/projections/retry")(
    pathParams,
  );
  ...
  const context = {
    ...
    operationID: "lucid_retry_passport_projections",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.LucidRetryPassportProjectionsResponse$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
