 ...
 */
export function memoryLucidGetMemoryStats(
  client: RaijinLabsLucidAiCore,
  request: operations.LucidGetMemoryStatsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.LucidGetMemoryStatsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.LucidGetMemoryStatsRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/v1/memory/stats/{agent_id}")(pathParams);
  ...
  const context = {
    ...
    operationID: "lucid_get_memory_stats",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.LucidGetMemoryStatsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
