...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.LucidRecallMemoryRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/v1/memory/recall")();
  ...
  const context = {
    ...
    operationID: "lucid_recall_memory",
    oAuth2Scopes: null,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.MemoryRecallResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
